...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const enabled = body.enabled === undefined ? null : Number(body.enabled) ? 1 : 0;
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      ...
      WHERE id = ?
      `,
      [dayOfWeek, timeSlot, dueTime, notes === null ? null : notes.slice(0, 255), enabled, id],
    );
    ...
    return NextResponse.json({ ok: false, message: 'Unable to update drink schedule.' }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const [result] = await pool.execute<ResultSetHeader>(`DELETE FROM drink_schedules WHERE id = ?`, [id]);
    if (result.affectedRows === 0) {
      return NextResponse.json({ ok: false, message: 'Schedule not found.' }, { status: 404 });
    }
    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json({ ok: false, message: 'Unable to delete drink schedule.' }, { status: 500 });
  }
}
