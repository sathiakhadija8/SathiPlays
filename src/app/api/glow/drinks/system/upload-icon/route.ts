...
export async function POST(request: Request) {
  try {
    ...
    const uploadsDir = path.join(process.cwd(), 'public', 'uploads', 'glow', 'drinks');
    await mkdir(uploadsDir, { recursive: true });

    const buffer = Buffer.from(await file.arrayBuffer());
    await writeFile(path.join(uploadsDir, uniqueName), buffer);

    return NextResponse.json({ ok: true, icon_image_path: `/uploads/glow/drinks/${uniqueName}` });
  } catch (error) {
    if (error instanceof FormDataRequestError) {
      return NextResponse.json({ ok: false, message: error.message }, { status: 400 });
    }
    return NextResponse.json({ ok: false, message: 'Unable to upload drink icon.' }, { status: 500 });
  }
}
