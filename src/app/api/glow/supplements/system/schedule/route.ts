...
export async function POST(request: Request) {
  try {
    ...
    const supplementId = Number(body.supplement_id);
    ...
    if (!Number.isInteger(supplementId) || supplementId <= 0) {
      return NextResponse.json({ ok: false, message: 'supplement_id is required.' }, { status: 400 });
    }
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `
      INSERT INTO supplement_schedules (supplement_id, day_of_week, time_slot, due_time, notes, enabled, is_active)
      VALUES (?, ?, ?, ?, ?, ?, ?)
      `,
      [
        supplementId,
    ...
    return NextResponse.json({ ok: false, message: 'Unable to create schedule.' }, { status: 500 });
  }
}
