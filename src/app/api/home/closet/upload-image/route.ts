...
export async function POST(request: Request) {
  try {
    ...
    const uploadsDir = path.join(process.cwd(), 'public', 'uploads', 'home', 'closet');
    await mkdir(uploadsDir, { recursive: true });

    const buffer = Buffer.from(await file.arrayBuffer());
    await writeFile(path.join(uploadsDir, uniqueName), buffer);

    return NextResponse.json({ ok: true, image_path: `/uploads/home/closet/${uniqueName}` });
  } catch (error) {
    if (error instanceof FormDataRequestError) {
      return NextResponse.json({ ok: false, message: error.message }, { status: 400 });
    }
    return NextResponse.json({ ok: false, message: 'Unable to upload closet image.' }, { status: 500 });
  }
}
