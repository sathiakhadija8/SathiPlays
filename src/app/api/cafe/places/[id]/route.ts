...
export async function PATCH(request: Request, context: { params: { id: string } }) {
  try {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `UPDATE cafe_places
       SET name = ?, location = ?, visited_date = ?, rating = ?, note = ?, images_json = ?, tag = ?
       WHERE id = ? AND user_id = ?`,
      [name, location, date, rating, note || null, JSON.stringify(images), tag, id, userId],
    );
    ...
    return NextResponse.json({ ok: false, message: 'Unable to update place.' }, { status: 500 });
  }
}
...
export async function DELETE(_: Request, context: { params: { id: string } }) {
  try {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `DELETE FROM cafe_places WHERE id = ? AND user_id = ?`,
      [id, userId],
    );
    if (!result.affectedRows) {
      return NextResponse.json({ ok: false, message: 'Place not found.' }, { status: 404 });
    ...
    return NextResponse.json({ ok: false, message: 'Unable to delete place.' }, { status: 500 });
  }
}
