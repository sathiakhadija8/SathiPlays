...
export async function PATCH(request: Request, context: { params: { id: string } }) {
  try {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `UPDATE travel_trips
       SET city = ?, country = ?, start_date = ?, end_date = ?, status = ?, cover_image = ?,
           planned_budget = ?, spent_budget = ?, reflection = ?, gallery_json = ?, places_json = ?
    ...
    return NextResponse.json({ ok: true, duration_days: computeDurationDays(startDate, endDate) });
  } catch (error) {
    if (error instanceof TravelImagePersistError) {
      return NextResponse.json({ ok: false, message: error.message }, { status: 400 });
    }
    return NextResponse.json({ ok: false, message: 'Unable to update trip.' }, { status: 500 });
  }
}
...
export async function DELETE(_: Request, context: { params: { id: string } }) {
  try {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `DELETE FROM travel_trips WHERE id = ? AND user_id = ?`,
      [id, userId],
    );
    if (!result.affectedRows) {
      return NextResponse.json({ ok: false, message: 'Trip not found.' }, { status: 404 });
    ...
    return NextResponse.json({ ok: false, message: 'Unable to delete trip.' }, { status: 500 });
  }
}
