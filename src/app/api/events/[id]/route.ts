...
export async function PATCH(request: Request, context: { params: { id: string } }) {
  try {
    ...
    if (!startAt) {
      return NextResponse.json({ ok: false, message: 'start_at must be DATETIME string.' }, { status: 400 });
    }
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `UPDATE events
       SET title = ?, start_at = ?, end_at = ?, location = ?, notes = ?, category = ?
       WHERE id = ?`,
      [
        title,
        startAt,
    ...
    console.error('[events][PATCH] failed to update event', error);
    return NextResponse.json({ ok: false, message: 'Unable to update event right now.' }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, context: { params: { id: string } }) {
  try {
    ...
    const [result] = await pool.execute<ResultSetHeader>('DELETE FROM events WHERE id = ?', [id]);
    ...
    console.error('[events][DELETE] failed to delete event', error);
    return NextResponse.json({ ok: false, message: 'Unable to delete event right now.' }, { status: 500 });
  }
}
